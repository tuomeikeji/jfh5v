	...
	export default {
		...
		methods:{
				...
				uni.chooseImage({
					...
					success: function(e){
						...
						if(_self.serverUrl){
							...
							var promiseWorkList = []
							var keyname = (_self.fileKeyName ? _self.fileKeyName : 'file')//upload-images
			...
			previewImage: function(e){
				var imageIndex = e.currentTarget.dataset.index
				console.log('插件中this.value',this.value);
				
				var urls = [];
				for (var i = 0; i < this.value.length; i++) {
					urls.push(JSON.parse(this.value[i]).data) 
				}
				console.log('插件中uploadUrls',urls);
				
				uni.previewImage({
					current: JSON.parse(this.value[imageIndex]).data,
					indicator: "number",
					loop: "true",
					urls:urls
				})
