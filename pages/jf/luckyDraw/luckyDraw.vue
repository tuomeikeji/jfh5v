<template>
	<view class="conbox">
		<view class="container">
			<!-- 背景 -->
			<image src="/static/img/luckyDraw/bg.png" class="cont" mode=""></image>
			<image src="/static/img/luckyDraw/caidai.png" class="caidai" mode=""></image>
			<view class="header">
				<view class="header-title">
					<view class="left">
						可用积分： <text style="color: #E4431A;">{{userScore}}</text>
					</view>
					<view class="right" @click="gotomyPrize">
						我的奖品 >
					</view>
				</view>
			</view>
						...
						<view class="canvas-list">
							<view class="canvas-item" :style="[{zIndex:index2}]" v-for="(iteml,index2) in awardsList" :key="index2">
								<view class="canvas-item-text" :style="[{transform:'rotate('+iteml.turn+')'}]">
									...
									<image class="canvas-item-text-img" src="/static/img/luckyDraw/xiaolian.png" v-if="iteml.type == 3"></image>
									<image class="canvas-item-text-img" src="/static/img/luckyDraw/youhuiquan.png" v-if="iteml.type == 2"></image>
									<image class="canvas-item-text-img" src="/static/img/luckyDraw/jingyan.png" v-if="iteml.type == 1"></image>
									<image class="canvas-item-text-img" src="/static/img/luckyDraw/jifenimg.png" v-if="iteml.type == 0"></image>
								</view>
							</view>
						</view>
					</view>

					<view @click="playReward" class="canvas-btn" v-bind:class="btnDisabled">开始 </view>
				</view>
			</view>
			...
			<view class="guize">
				...
				<view class="g_item">
					1.每抽奖一次消耗{{playScore}}积分；
				</view>
				<view class="g_item">
					2.用户获得的奖品，可在“我的奖品”里查看；
				</view>
				<view class="g_item">
					3.积分类奖品直接加入用户的原有积分中，非积分类奖品请找相关负责人兑换。
				</view>
			</view>
			<!-- 我的奖品 -->
	...
	export default {
		...
		methods: {
			getUserScore(){
				...
				.then((res)=>{
					console.log('success_用户可用积分----', res.data.data);
			...
			getmyPrize(){
				var that = this
				that.axios.post('/api/Turntable/myPrize', {
					page: 1,
					num:1000
			...
			initdata:function(){
				...
				that.axios({
				  url: that.GLOBAL.domain + '/integralDraw/find',
				  method: 'POST',
				  dataType: 'json',
				  header:{
					'content-type':'application/x-www-form-urlencoded'
				...
				.then(function(res) {
					console.log("success_初始化转盘_/integralDraw/find",res);
					that.GLOBAL.successHttp(res)
					that.awardsConfig = res.data.data;
					that.playScore = res.data.data.Prizeidscore;//玩一次消耗多少积分
					// that.chishu = data.luckdraw;
					// 获取奖品的个数
					let awarrlength = that.awardsConfig.gzdList.length
					// push 谢谢参与奖项
					// for (var i = 0; i <= 3 * 2; i++) {
					// 	if (i % 3 == 0) {
					// 		that.thanksarr.push(i)
					// 		that.awardsConfig.gzdList.splice(i, 0, {
					// 			name: '谢谢参与',
					// 			type: 0
					// 		});
					// 	}
					// }
					// 为每一项追加index属性
					that.awardsConfig.gzdList.forEach(function(element, index) {
						element.index = index
					})
			...
			drawAwardRoundel: function() {
				// 拿到奖品列表
				var awards = this.awardsConfig.gzdList;
				var awardsList = [];
				// 每份奖品所占角度
				...
				for (var i = 0; i < awards.length; i++) {
					awardsList.push({
						...
						// type: awards[i].type,
						amount:awards[i].amount,//奖品的剩余数量
						id:awards[i].id
					})
				}
			...
			playReward: function() {
				...
				// 获取奖品
				that.axios({
				  url: that.GLOBAL.domain + '/integralDraw/saveIntegralDraw',
				  method: 'POST',
				  dataType: 'json',
				  header:{
					'content-type':'application/x-www-form-urlencoded'
				...
				.then(function(data) {
					console.log('success_/saveIntegralDraw_开始抽奖',data);
					
					if(typeof(data.data.data) != 'number'){
						console.log('进入了积分不够')
						uni.showModal({
							content:data.data.data, //本次抽奖积分不够
							showCancel:false
						})
						return false;
					}
					
					var awardIndex = data.data.data; //奖品列表的下标 从0开始
					...
					// })
					//中奖index  
					var awardsNum = that.awardsConfig.gzdList; //转盘分割数
					...
					var awardName = awardsConfig.gzdList[awardIndex].name;
					var awardType = 1; 
					if(awardName.indexOf("参与") != -1 || awardName.indexOf("遗憾") != -1){
						awardType = 0;
					}
					...
					if (awardType != 0) {
						setTimeout(function() {
							uni.showModal({
								title: '恭喜',
								content: '获得' + (awardsConfig.gzdList[awardIndex].name),
								showCancel: false,
								success:function(){
									// 置空style  否则动画不生效
									setTimeout(function(){
							...
							that.btnDisabled = '';
							that.getUserScore(); //重新获取用户积分
						}.bind(that), duration);
					} else {
						setTimeout(function() {
							uni.showModal({
								title: '很遗憾',
								content: '没中奖 ' + (awardsConfig.gzdList[awardIndex].name),
								showCancel: false,
								success:function(){
									// 置空style  否则动画不生效
									setTimeout(function(){
			...
			gotomyPrize(){
				uni.navigateTo({
					url:'/pages/jf/luckyDraw/log/myPrize'
				})
			}
		}
...
<style scoped>
	...
	.canvas-container {
		...
		width: 600upx;
		height: 600upx;
		background: url(../../../static/img/luckyDraw/circle.png) no-repeat;
