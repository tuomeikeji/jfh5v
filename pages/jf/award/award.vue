<!-- 工作台::自由奖励 -->
<template>
	<view class="uni-page-body" style="padding-bottom: 60upx;">
		<form @submit="formSubmit" class="page-section">
			<view class="page-section-title"><text class="uni-text-red">*</text>标题：</view>
			...
			<view class="clear" style="padding: 30upx;">
				...
				<input class="page-section-input" type="text" name="points" style="border: 1px solid #EEEEEE;padding: 0;"/>
			</view>
			<view class="form-line"></view>
		...
		methods:{
			...
			getApprover(){
				...
				uni.request({
				  ...
				  success: (res) => { if ((res.data.code != 0 && !res.data.code ) || res.data.code == 1001) {
					  uni.showToast({ title: res.data.msg, duration: 3000,icon:'none' });
					  uni.reLaunch({ url: '/pages/jf/login/login' }); return}
			...
			formSubmit: function(e) {
				...
				var typeId = that.types[that.arrIndexType].id;//积分
				...
				uni.request({
				  url: this.GLOBAL.domain + '/free/freeIntegralApprover',
				  method: 'POST',
				  dataType: 'json',
				  header:{
						'content-type':'application/x-www-form-urlencoded'
